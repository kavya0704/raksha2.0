...
export default function TacticalMap({ cameras = [], alerts = [], onSelectCamera }) {
  ...
  const [mapMode, setMapMode] = useState('SATELLITE'); // 'SATELLITE' or 'RADAR'
  const [radarAngle, setRadarAngle] = useState(0);

  // Animated Radar Sweep when in Radar mode
  useEffect(() => {
  ...
  // Initialize and update Leaflet Interactive GIS Map
  useEffect(() => {
    ...
    try {
      // Centered at Sikkim / Nathu La Border Sector
      ...
      // Free high-detail OpenStreetMap tile layer with tactical contrast
      L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
        maxZoom: 19,
        className: 'tactical-map-tiles'
      }).addTo(map);

      // Red Sterile Perimeter Buffer Polygon (Zero Tolerance)
      ...
      borderPoly.bindTooltip("STERILE BORDER BUFFER ZONE (ZERO TOLERANCE)", { 
        className: 'tactical-tooltip',
        permanent: false 
      });

      // Red Horizontal Tripwire Line Alpha
      const tripwireLine = L.polyline([
        [27.388, 88.820],
        [27.385, 88.848]
      ], {
        color: '#FFB4AB',
      ...
      tripwireLine.bindTooltip("LINE OF CONTROL TRIPWIRE ALPHA", { 
        className: 'tactical-tooltip',
        permanent: false 
      });

      // Outpost Camera Markers
      cameras.forEach((cam) => {
      ...
      console.warn("Leaflet interactive map initialization notice:", err);
    }
  ...
  }, [cameras, alerts, onSelectCamera, mapMode]);
  ...
  return (
    <div className="relative w-full h-full min-h-[220px] bg-[#070b0e] rounded border border-[#3c494a] overflow-hidden select-none flex flex-col justify-between">
      ...
      <div className="absolute top-2 left-3 right-3 z-[500] flex items-center justify-between pointer-events-none">
        ...
        {/* View Mode Toggle Button: Geographic Satellite Map vs Radar Grid */}
          ...
          {cameras.map((cam, idx) => {
            ...
            return (
              <g 
                ...
                onClick={() => { if (onSelectCamera) onSelectCamera(cam); }}
              >
                {isAlert && (
